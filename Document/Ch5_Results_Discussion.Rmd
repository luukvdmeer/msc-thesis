...
The spatial and temporal patterns of the forecast RMSE values are shown together in Figure 5.9, where each point represents the RMSE of a single forecasted test point. The points are plotted with time, with the time values corresponding to the starting time of each forecast. The shaded areas are weekends. The patterns that show endorse the findings above. In the Bayview cluster, only a few forecasts are made, all with a relatively low RMSE, and little difference between DBAFS and the Naïve method. To less extent, the same yields for the Presidio cluster. In the Downtown and Residential clusters, where the majority of the forecasts are made, the average RMSE of the forecasts with DBAFS is only slightly lower than of those with the Naïve method, but they are clearly more compact. Mainly later on the day, the Naïve method starts to give higher errors. Especially in the Downtown cluster, the RMSE's of forecasts in the weekend are lower than during weekdays, both for DBAFS and the Naïve method.
...
Figure 5.10a shows the average RMSE per hour of the day. During the night time, the errors are low for both DBAFS and the Naïve method, followed by a strong increase during the morning peak hour. The afternoon peak hour shows lower RMSE's, and again more similarity between the two methods. During times where the average RMSE is high, and apparently, the data get more complex and less forecastable, DBAFS performs considerably better than the naïve method. 

Figure 5.10b shows the average RMSE per forecast lag. For the naïve method, the RMSE at lag $0$ is always $0$, since it simply forecast the last known value of the time series. For DBAFS, this it not the case, since it add the corresponding seasonal term of that time stamp to the forecast, which turns out to be not reliable enough to be appropriate everywhere. Therefore, in short term forecasting, the Naïve method performs slightly better, although for both methods the RMSE increases very fast during the first lags. After about two hours, the increase of RMSE for DBAFS starts to get flatter, while for the Naïve method, it keeps increasing. Further into the furture, the average RMSE's converge to the mean absolute error of the corresponding methods.
