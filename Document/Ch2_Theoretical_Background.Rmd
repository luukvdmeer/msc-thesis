...
`*NOTE: Describe the frequency domain approach to time series analysis, where time series are analyzed with spectral analysis methods. Most importantly, show how the spectral entropy is calculated.*`
...
`*NOTE: Insert figure that summarizes the Hyndman-Khandakar algorithm*`
...
Recall that in Definition 1, a time series was defined as a collection of random variables. In fact, to state it statistically correct, it is the distribution of the random variable $h$ time lags ahead that is forecasted, rather than an individual value. This distribution is referred to as the forecast distribution, and the forecasted value, also known as the point forecast, is then taken to be the mean of the forecast distribution. In Equation 2.x and 2.x, $\hat\sigma_{h}$ is the estimated standard deviation of the forecasted distribution, assuming it is a normal distribution with mean ${y}_{t+h}$ and variance $\sigma_{h}^{2}$. When $h = 1$, the residual standard deviation $\sigma_{\epsilon}$ is a good estimate for $\sigma_{h}$. However, for $h > 1$, computation get more complex. For a detailed description, see @shumway2011, section 3.5. 

### 2.4.2.6 Accuracy evaluation
`*NOTE: Provide formulas for calculating the RMSE and MAE of forecasts. Also mention the RMSLE*`

### 2.4.2.7 Transformations
`*NOTE: Discuss transformations. Why are they sometimes needed? Focus in particular on log transformation. Also, explain why bias adjustment is needed when transforming back to the original scale, and give formula for this bias adjustment as used in R*`

## 2.4.3 Naïve forecasts
`*NOTE: Explain the concept of naïve forecasts*`

## 2.4.4 Seasonality
`*NOTE: Explain seasonal ARIMA(p,d,q)(P,D,Q) models, en explain why they are not suitable for data with long and complex seasonal patterns. Describe the approach of Hyndman and Athanasopolous of first decomposing the data, then forecasting the non-seasonal part with a time series model, and the seasonal part with a seasonal naïve method.*`
