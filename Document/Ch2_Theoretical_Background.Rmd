...
$$ 
\nabla^{d}y_{t+h} = \hat{c} + \hat\phi_{1}\nabla^{d}y_{t+h-1} + ... + \hat\phi_{p}\nabla^{d}y_{t+h-p} +\hat\theta_{1}\epsilon_{t+h-1} + ... + \hat\theta_{q}\epsilon_{t+h-q} + \epsilon_{t+1} 
$$

Where $h$ is the forecasting window, i.e. the number of time lags into the future at which the forecast is made, $p$, $d$ and $q$ are known and constant, and $\hat{c}$, $\hat\phi_{1},...,\hat\phi_{p}$ and $\hat\theta_{1},...,\hat\theta_{q}$ are the estimated parameter values, which are also constant.
