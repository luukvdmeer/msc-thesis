...
Where $V(\Lambda_{k})$ is the Dunn Index of a partition $\Lambda_{k}$ with $k$ clusters, $1 \leq i, j, l \leq k$, $D(C_{i}, C_{j})$ is the shortest Euclidean distance between a data point in cluster $C_{i} \in \Lambda_{k}$ and a data point in cluster $C_{j} \in \Lambda_{k}$, given that $C_{i} \neq C_{j}$, and $diam(C_{l})$ is the largest Euclidean distance  between two data points in cluster $C_{l} \in \Lambda_{k}$. To find the optimal partition $\Lambda_{k}^*$, the Dunn Index should be maximized.
